...
public class DashboardDao {
    public Map<String, Integer> getSummaryStats() throws SQLException {
        ...
        stats.put("Phiếu mượn đang mượn",
                count("SELECT COUNT(*) AS total FROM PhieuMuon WHERE TrangThai = N'BORROWING'"));
    ...
    public Map<String, Integer> getBookStatusBreakdown() throws SQLException {
        return groupedCounts(
                "SELECT COALESCE(TrangThai, N'UNKNOWN') AS label, COUNT(*) AS total FROM Sach GROUP BY COALESCE(TrangThai, N'UNKNOWN') ORDER BY total DESC");
    }

    public Map<String, Integer> getLoanStatusBreakdown() throws SQLException {
        return groupedCounts(
                "SELECT COALESCE(TrangThai, N'UNKNOWN') AS label, COUNT(*) AS total FROM PhieuMuon GROUP BY COALESCE(TrangThai, N'UNKNOWN') ORDER BY total DESC");
    }

    public Map<String, Integer> getReaderTypeBreakdown() throws SQLException {
        return groupedCounts(
                "SELECT COALESCE(LoaiDocGia, N'Không rõ') AS label, COUNT(*) AS total FROM DocGia GROUP BY COALESCE(LoaiDocGia, N'Không rõ') ORDER BY total DESC");
    }

    public Map<String, Integer> getReaderActivityBreakdown() throws SQLException {
        return groupedCounts(
                "SELECT CASE WHEN COALESCE(TrangThai, 1) = 1 THEN N'Hoạt động' ELSE N'Ngừng hoạt động' END AS label, COUNT(*) AS total FROM DocGia GROUP BY CASE WHEN COALESCE(TrangThai, 1) = 1 THEN N'Hoạt động' ELSE N'Ngừng hoạt động' END ORDER BY total DESC");
    }

    public Map<java.time.LocalDate, Integer> getLoanTrend(java.time.LocalDate from, java.time.LocalDate to)
            throws SQLException {
        /**
    ...
    public Map<String, Integer> getTopReaders(int limit) throws SQLException {
        ...
        String sql = "SELECT TOP (" + limit
                + ") dg.HoTen AS name, SUM(CASE WHEN pm.NgayTraThuc <= pm.NgayTraDuKien THEN 10 ELSE -2 END) AS score FROM PhieuMuon pm JOIN DocGia dg ON pm.MaDocGia = dg.MaDocGia WHERE pm.NgayTraThuc IS NOT NULL GROUP BY dg.HoTen ORDER BY score DESC";
        DbTemplate.query(sql, rs -> {
